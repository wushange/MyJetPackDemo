...
//    private static int[] sLandscapeRealSizeCache = null;
	...
	public static int getScreenHeight(Context context) {
		...
		if (AUIDeviceHelper.isXiaomi() && xiaomiNavigationGestureEnabled(context)) {
...
//        if (AUIDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
...
//        }
	...
	public static int getUsefulScreenWidth(Activity activity) {
		return getUsefulScreenWidth(activity, AUINotchHelper.hasNotch(activity));
	}

	public static int getUsefulScreenWidth(View view) {
		return getUsefulScreenWidth(view.getContext(), AUINotchHelper.hasNotch(view));
	}
	...
	public static int getUsefulScreenWidth(Context context, boolean hasNotch) {
		...
		if (!hasNotch) {
			if (isLandscape && AUIDeviceHelper.isEssentialPhone()
				...
				result -= 2 * AUIStatusBarHelper.getStatusbarHeight(context);
			}
			return result;
		}
		if (isLandscape) {
			// 华为挖孔屏横屏时，会把整个 window 往后移动，因此，可用区域减小
			if (AUIDeviceHelper.isHuawei() && !AUIDisplayHelper.huaweiIsNotchSetToShowInSetting(context)) {
				result -= AUINotchHelper.getNotchSizeInHuawei(context)[1];
			}
	...
	public static int getUsefulScreenHeight(Activity activity) {
		return getUsefulScreenHeight(activity, AUINotchHelper.hasNotch(activity));
	}

	public static int getUsefulScreenHeight(View view) {
		return getUsefulScreenHeight(view.getContext(), AUINotchHelper.hasNotch(view));
	}
	...
	private static int getUsefulScreenHeight(Context context, boolean hasNotch) {
		...
		if (!hasNotch) {
			if (isPortrait && AUIDeviceHelper.isEssentialPhone()
					&& Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
				// https://arstechnica.com/gadgets/2017/09/essential-phone-review-impressive-for-a-new-company-but-not-competitive/
				// 这里说挖孔屏是状态栏高度的两倍
				result -= 2 * AUIStatusBarHelper.getStatusbarHeight(context);
