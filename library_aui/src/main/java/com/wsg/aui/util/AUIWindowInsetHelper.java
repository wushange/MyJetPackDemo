...
import com.wsg.aui.widget.INotchInsetConsumer;
import com.wsg.aui.widget.IWindowInsetLayout;
...
public class AUIWindowInsetHelper {
    ...
    public AUIWindowInsetHelper(ViewGroup viewGroup, IWindowInsetLayout windowInsetLayout) {
        mWindowInsetLayoutWR = new WeakReference<>(windowInsetLayout);
        KEYBOARD_HEIGHT_BOUNDARY = AUIDisplayHelper.dp2px(viewGroup.getContext(), 100);

        if (AUINotchHelper.isNotchOfficialSupport()) {
    ...
    public boolean defaultApplySystemWindowInsets19(ViewGroup viewGroup, Rect insets) {
        ...
        if (insets.bottom >= KEYBOARD_HEIGHT_BOUNDARY) {
            AUIViewHelper.setPaddingBottom(viewGroup, insets.bottom);
            viewGroup.setTag(R.id.aui_window_inset_keyboard_area_consumer, KEYBOARD_CONSUMER);
            insets.bottom = 0;
        } else {
            viewGroup.setTag(R.id.aui_window_inset_keyboard_area_consumer, null);
            AUIViewHelper.setPaddingBottom(viewGroup, 0);
        }
    ...
    public boolean defaultApplySystemWindowInsetsCompat(ViewGroup viewGroup, WindowInsetsCompat insets) {
        ...
        if (insets.getSystemWindowInsetBottom() >= KEYBOARD_HEIGHT_BOUNDARY) {
            showKeyboard = true;
            AUIViewHelper.setPaddingBottom(viewGroup, insets.getSystemWindowInsetBottom());
            viewGroup.setTag(R.id.aui_window_inset_keyboard_area_consumer, KEYBOARD_CONSUMER);
        } else {
            AUIViewHelper.setPaddingBottom(viewGroup, 0);
            viewGroup.setTag(R.id.aui_window_inset_keyboard_area_consumer, null);
        ...
        for (int i = 0; i < viewGroup.getChildCount(); i++) {
            ...
            if (AUINotchHelper.needFixLandscapeNotchAreaFitSystemWindow(viewGroup) &&
                    viewGroup.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE) {
                insetLeft = Math.max(insetLeft, AUINotchHelper.getSafeInsetLeft(viewGroup));
                insetRight = Math.max(insetRight, AUINotchHelper.getSafeInsetRight(viewGroup));
            }
    ...
    public static View findKeyboardAreaConsumer(@NonNull View view) {
        while (view != null) {
            Object tag = view.getTag(R.id.aui_window_inset_keyboard_area_consumer);
