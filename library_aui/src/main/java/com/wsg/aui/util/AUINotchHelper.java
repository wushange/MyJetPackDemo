...
public class AUINotchHelper {

    private static final String TAG = "AUINotchHelper";
    ...
    public static boolean has3rdNotch(Context context){
        if (AUIDeviceHelper.isHuawei()) {
            return hasNotchInHuawei(context);
        } else if (AUIDeviceHelper.isVivo()) {
            return hasNotchInVivo(context);
        } else if (AUIDeviceHelper.isOppo()) {
            return hasNotchInOppo(context);
        } else if (AUIDeviceHelper.isXiaomi()) {
    ...
    private static Rect get3rdSafeInsetRect(Context context){
        // 全面屏设置项更改
        if (AUIDeviceHelper.isHuawei()) {
            boolean isHuaweiNotchSetToShow = AUIDisplayHelper.huaweiIsNotchSetToShowInSetting(context);
    ...
    private static Rect getRectInfoRotation0(Context context) {
        ...
        if (AUIDeviceHelper.isVivo()) {
            ...
            rect.top = AUIStatusBarHelper.getStatusbarHeight(context);
            rect.bottom = 0;
        } else if (AUIDeviceHelper.isHuawei()) {
    ...
    private static Rect getRectInfoRotation90(Context context) {
        ...
        if (AUIDeviceHelper.isVivo()) {
            ...
            rect.left = AUIStatusBarHelper.getStatusbarHeight(context);
            rect.right = 0;
        } else if (AUIDeviceHelper.isHuawei()) {
    ...
    private static Rect getRectInfoRotation180(Context context) {
        ...
        if (AUIDeviceHelper.isVivo()) {
            ...
            rect.top = 0;
            rect.bottom = AUIStatusBarHelper.getStatusbarHeight(context);
        } else if (AUIDeviceHelper.isHuawei()) {
            int[] notchSize = getNotchSizeInHuawei(context);
            rect.top = 0;
    ...
    private static Rect getRectInfoRotation270(Context context) {
        ...
        if (AUIDeviceHelper.isVivo()) {
            ...
            rect.right = AUIStatusBarHelper.getStatusbarHeight(context);
            rect.left = 0;
        } else if (AUIDeviceHelper.isHuawei()) {
            if (sHuaweiIsNotchSetToShow) {
    ...
    public static int getNotchHeightInXiaomi(Context context) {
        ...
        return AUIDisplayHelper.getStatusBarHeight(context);
    }

    public static int getNotchWidthInVivo(Context context){
        return AUIDisplayHelper.dp2px(context, 100);
    }

    public static int getNotchHeightInVivo(Context context){
        return AUIDisplayHelper.dp2px(context, 27);
    }
    ...
    public static boolean needFixLandscapeNotchAreaFitSystemWindow(View view){
       return (AUIDeviceHelper.isXiaomi() || AUIDeviceHelper.isVivo()) && AUINotchHelper.hasNotch(view);
    }
