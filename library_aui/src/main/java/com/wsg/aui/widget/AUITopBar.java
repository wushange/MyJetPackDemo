...
import com.wsg.aui.util.AUIDisplayHelper;
import com.wsg.aui.util.AUIDrawableHelper;
import com.wsg.aui.util.AUILangHelper;
 ...
 * </ul>
 */
public class AUITopBar extends RelativeLayout {
    ...
    public AUITopBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initVar();
        init(context, attrs, defStyleAttr);
    }

    public AUITopBar(Context context, AttributeSet attrs) {
    ...
    public AUITopBar(Context context) {
        this(context, null);
    }

    // ========================= title 相关的方法

    // 这个构造器只用于AUI内部，不开放给外面用，目前用于AUITopBarLayout
    AUITopBar(Context context, boolean inTopBarLayout) {
        super(context);
        initVar();
        if (inTopBarLayout) {
            int transparentColor = ContextCompat.getColor(context, R.color.aui_config_color_transparent);
            mTopBarSeparatorColor = transparentColor;
            mTopBarSeparatorHeight = 0;
            mTopBarBgColor = transparentColor;
        } else {
            init(context, null, R.attr.AUITopBarStyle);
        }
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.AUITopBar, defStyleAttr, 0);
    ...
    void getCommonFieldFormTypedArray(Context context, TypedArray array) {
        mLeftBackDrawableRes = array.getResourceId(R.styleable.AUITopBar_aui_topbar_left_back_drawable_id, R.drawable.aui_icon_topbar_back);
        mTitleGravity = array.getInt(R.styleable.AUITopBar_aui_topbar_title_gravity, Gravity.CENTER);
        mTitleTextSize = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_title_text_size, AUIDisplayHelper.sp2px(context, 17));
        mTitleTextSizeWithSubTitle = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_title_text_size, AUIDisplayHelper.sp2px(context, 16));
        mSubTitleTextSize = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_subtitle_text_size, AUIDisplayHelper.sp2px(context, 11));
        mTitleTextColor = array.getColor(R.styleable.AUITopBar_aui_topbar_title_color, AUIResHelper.getAttrColor(context, R.attr.aui_config_color_gray_1));
        mSubTitleTextColor = array.getColor(R.styleable.AUITopBar_aui_topbar_subtitle_color, AUIResHelper.getAttrColor(context, R.attr.aui_config_color_gray_4));
        mTitleMarginHorWhenNoBtnAside = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_title_margin_horizontal_when_no_btn_aside, 0);
        mTitleContainerPaddingHor = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_title_container_padding_horizontal, 0);
        mTopBarImageBtnWidth = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_image_btn_width, AUIDisplayHelper.dp2px(context, 48));
        mTopBarImageBtnHeight = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_image_btn_height, AUIDisplayHelper.dp2px(context, 48));
        mTopBarTextBtnPaddingHor = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_text_btn_padding_horizontal, AUIDisplayHelper.dp2px(context, 12));
        mTopBarTextBtnTextColor = array.getColorStateList(R.styleable.AUITopBar_aui_topbar_text_btn_color_state_list);
        mTopBarTextBtnTextSize = array.getDimensionPixelSize(R.styleable.AUITopBar_aui_topbar_text_btn_text_size, AUIDisplayHelper.sp2px(context, 16));
    ...
    public TextView setTitle(String title) {
        ...
        if (AUILangHelper.isNullOrEmpty(title)) {
...
//            mTitleView = isEmoji ? new EmojiconTextView(getContext()) : new TextView(getContext());
    ...
    private void updateTitleViewStyle() {
        if (mTitleView != null) {
            if (mSubTitleView == null || AUILangHelper.isNullOrEmpty(mSubTitleView.getText())) {
    ...
    public void setSubTitle(String subTitle) {
        ...
        if (AUILangHelper.isNullOrEmpty(subTitle)) {
            titleView.setVisibility(GONE);
        } else {
            titleView.setVisibility(VISIBLE);
        }
    ...
    private TextView getSubTitleView() {
        if (mSubTitleView == null) {
            ...
            LinearLayout.LayoutParams titleLp = generateTitleViewAndSubTitleViewLp();
            titleLp.topMargin = AUIDisplayHelper.dp2px(getContext(), 1);
    ...
    public Rect getTitleContainerRect() {
        ...
        if (mTitleContainerView == null) {
            ...
            AUIViewHelper.getDescendantRect(this, mTitleContainerView, mTitleContainerRect);
        }
    ...
    private LayoutParams generateTitleContainerViewLp() {
        return new LayoutParams(LayoutParams.MATCH_PARENT,
                AUIResHelper.getAttrDimen(getContext(), R.attr.aui_topbar_height));
    }
    ...
    public AUIAlphaImageButton addRightImageButton(int drawableResId, int viewId) {
        AUIAlphaImageButton rightButton = generateTopBarImageButton(drawableResId);
    ...
    public AUIAlphaImageButton addLeftImageButton(int drawableResId, int viewId) {
        AUIAlphaImageButton leftButton = generateTopBarImageButton(drawableResId);
     ...
     */
    private AUIAlphaImageButton generateTopBarImageButton(int imageResourceId) {
        AUIAlphaImageButton backButton = new AUIAlphaImageButton(getContext());
    ...
    public AUIAlphaImageButton addLeftBackImageButton() {
        return addLeftImageButton(mLeftBackDrawableRes, R.id.aui_topbar_item_left_back);
    }
    ...
    private int getTopBarHeight() {
        if (mTopbarHeight == -1) {
            mTopbarHeight = AUIResHelper.getAttrDimen(getContext(), R.attr.aui_topbar_height);
        }
        ...
        if (mTitleContainerView != null) {
            ...
            if ((mTitleGravity & Gravity.HORIZONTAL_GRAVITY_MASK) == Gravity.CENTER_HORIZONTAL) {
                ...
                if (mLeftViewList.isEmpty()) {
                    //左侧没有按钮，标题离左侧间距
                    titleContainerViewLeft += AUIResHelper.getAttrDimen(getContext(),
                            R.attr.aui_topbar_title_margin_horizontal_when_no_btn_aside);
                }
            }
